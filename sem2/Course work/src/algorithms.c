...
void * linear_search(const void *array, size_t size, const void *target_element, size_t type_size,
    ...
    if ((array == NULL && size != 0) || target_element == NULL || type_size == 0 || compare == NULL || found_size == NULL) {
    ...
    if (array == NULL) {
        // If the array is empty, there is nothing to do, terminating the function (returning NULL).
        return NULL;
    }
...
void * predicate_search(const void *array, size_t size, size_t type_size, size_t *found_size,
    ...
    if ((array == NULL && size != 0) || type_size == 0 || found_size == NULL || predicate == NULL) {
        // If not, interrupting the function (setting errno to EINVAL and returning NULL).
        errno = EINVAL;
...
int quicksort(void *array, size_t size, size_t type_size, compare_func_t compare) {
    ...
    if ((array == NULL && size != 0) || type_size == 0 || compare == NULL) {
        // If not, interrupting the function (setting errno to EINVAL and returning -1).
        errno = EINVAL;
        return -1;
    }
    ...
    if (array == NULL) {
        // If the array is empty, there is nothing to do, terminating the function (returning 0).
        return 0;
    }
